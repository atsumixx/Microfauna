...
def init_db():
    ...
    # Auto-fix: ensure items.id has a sequence (handles migration from plain int4)
    c.execute("""
        DO $$
        BEGIN
            IF NOT EXISTS (
...
def get_sale_data(sale_id):
    """Fetch full sale + items for receipt use."""
    conn = get_db()
    c = conn.cursor()
...
def api_item_sales():
    ...
    c.execute("""SELECT si.item_name,
                 ...
                 FROM sale_items si
                 GROUP BY si.item_name ORDER BY total_sales DESC""")
    data = [dict(row) for row in c.fetchall()]
    conn.close()
    return jsonify(data)
...
def api_monthly_comparison():
    ...
    c.execute("""SELECT to_char(date, 'YYYY-MM') as month, SUM(total) as amount
                 FROM sales GROUP BY to_char(date, 'YYYY-MM')""")
    sales_data = [dict(row) for row in c.fetchall()]

    c.execute("""SELECT to_char(date, 'YYYY-MM') as month, SUM(amount) as amount
                 FROM expenses GROUP BY to_char(date, 'YYYY-MM')""")
...
def view_receipt(sale_id):
    """Return sale data as JSON for receipt modal (re-viewable anytime)."""
...
def download_receipt(sale_id):
    """Download receipt as a plain-text .txt file."""
    data = get_sale_data(sale_id)
    if not data:
    ...
    for item in data['items']:
        lines.append(
            f"{item['name']:<20} {item['quantity']:>4} "
            f"P{item['price']:>6.2f} P{item['subtotal']:>6.2f}"
        )
    lines.append("-" * 40)
...
def add_sale():
    ...
    if request.method == 'POST':
        try:
            ...
            if sale_entries:
                c.execute(
                    "INSERT INTO sales (customer_name, date, total, notes) VALUES (%s,%s,%s,%s) RETURNING id",
                ...
                return jsonify({
                    ...
                    'items': [{'name': e[0], 'quantity': e[1],
                               'price': float(e[2]), 'subtotal': float(e[3])} for e in sale_entries],
                    'total': float(total)
                })
            else:
                conn.close()
...
def view_sales():
    ...
    for sale in sales:
        c.execute("""SELECT item_name as name, quantity, price, subtotal
                     FROM sale_items WHERE sale_id=%s""", (sale['id'],))
        items = c.fetchall()
...
def edit_expense(expense_id):
    ...
    if request.method == 'POST':
        try:
            ...
            date = request.form['date']
            notes = request.form.get('notes', '').strip()
            c.execute("""UPDATE expenses SET description=%s, amount=%s, category=%s, date=%s, notes=%s
                         WHERE id=%s""", (description, amount, category, date, notes, expense_id))
            conn.commit()
            conn.close()
            return redirect(url_for('view_expenses'))
        except Exception as e:
